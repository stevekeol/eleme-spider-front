...
// 获取爬虫总体信息
export const GET_SPIDER_SUMMARY_DATA = 'GET_SPIDER_SUMMARY_DATA';

export function getSpiderSummaryData() {
  const option = {
    url: '/getSpiderSummaryData'
  };
  return {
    type: GET_SPIDER_SUMMARY_DATA,
    payload: axios.get(option)
  };
}

// 获取城市爬取信息
export const GET_CITY_SPIDER_INFO = 'GET_CITY_SPIDER_INFO';

export function geCitySpiderInfo() {
  const option = {
    url: '/geCitySpiderInfo'
  };
  return {
    type: GET_CITY_SPIDER_INFO,
    payload: axios.get(option)
  };
}

// 部署新爬取任务
export const START_SPIDER_TASK = 'START_SPIDER_TASK';

export function startSpiderTask(spiderOptions) {
  // console.log('actions-spider:');
  // console.log(spiderOptions);

  const option = {
    url: '/startSpiderTask',
    params: {
      province: spiderOptions.province,
      city: spiderOptions.city,
      // county: spiderOptions.county
      latitude: spiderOptions.latitude,
      longitude: spiderOptions.longitude
    }
  };
  return {
    type: START_SPIDER_TASK,
    payload: axios.get(option)
  };
}
...
export function exportDataToExcel(downloadOptions) {
  console.log('actions-download:');
  console.log(downloadOptions);

  const option = {
    url: '/exportDataToExcel',
    params: {
      province: downloadOptions.province,
      city: downloadOptions.city,
      // county: downloadOptions.county
      // latitude: downloadOptions.latitude,
      // longitude: downloadOptions.longitude
    }
  };
  return {
    type: EXPORT_DATA_TO_EXCEL,
    payload: axios.get(option)
  };
}
